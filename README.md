...
Функция send_log_to_loki() отправляет каждое лог-событие через requests.post(). Класс LokiHandler(logging.Handler) автоматически перехватывает все записи логгера и дублирует их в Loki.
